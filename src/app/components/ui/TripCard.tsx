...
export default function TripCard({ trip, onBook }: Props) {
  return (
    <article
      className="bg-white border-3 border-[#ECECEC] rounded-3xl shadow-[0_2px_10px_rgba(0,0,0,0.08)] px-3 py-4"
      style={{ fontFamily: "Century Gothic, sans-serif" }}
    >
      <div className="flex gap-4 items-start">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="ml-2 space-y-2">
            <MetaRow
              icon={<MapPin className="w-[18px] h-[18px] text-[#28B872]" />}
              label="Location"
              value={trip.location}
            />
            <MetaRow
              icon={<Tag className="w-[18px] h-[18px] text-[#28B872]" />}
              label="Price"
              value={`${trip.priceBr.toLocaleString()} Br`}
            />
            <MetaRow
              icon={<Calendar className="w-[18px] h-[18px] text-[#28B872]" />}
              label="Days"
