...
import Detailfilter from "@/app/components/ui/Detailfilter"; // ← uses tourData prop
...
function fmtMoneyStr(n?: number | null): string {
  ...
  return new Intl.NumberFormat("en-ET", { maximumFractionDigits: 0 }).format(n as number);
}
    ...
    const normalized = val
      ...
      .map((x) => {
        ...
        return {
          ...
          activity: typeof o.activity === "string" ? o.activity : undefined,
        };
      });
...
export default function TourPage() {
  ...
  const backToHome = () => router.push("/pages/home");
    ...
    (async () => {
      ...
      if (!error && row) {
        ...
        const resolved = await Promise.all(
          rawPhotos.map((p) => toImageUrlFromStorageKey(p).catch(() => p))
        );
        setImages(resolved);
        ...
        const old = disc && disc > 0 && typeof price === "number" ? price : undefined;

        setDiscountPercent(disc);
        setPriceNowStr(now !== undefined ? `${fmtMoneyStr(now)} Br` : "");
        setPriceOldStr(old !== undefined ? `${fmtMoneyStr(old)} Br` : "");
        ...
        setTourData({
          ...
          essentialEquipment: getStringArray(r.essentialEquipment) ?? [],
  ...
  return (
    <div className="w-full mx-auto bg-white">
      ...
      <div className="relative h-[280px] sm:h-[350px] md:h-[400px] w-full overflow-hidden">
        {(loading || Images.length === 0) && <Skeleton className="absolute inset-0 h-full w-full" />}
        ...
        <div className="absolute top-4 left-4 flex items-center gap-2 z-10">
          <button className="p-2 bg-white/80 rounded-full hover:bg-white">
            <ChevronLeft size={20} className="text-green-500" onClick={backToHome} />
          </button>
        </div>
          ...
          {Images.map((_, index) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-all ${
                currentImage === index ? "bg-green-500" : "bg-white/50 hover:bg-white/70"
              }`}
            />
          ))}
        </div>
        ...
        <div className="bg-white relative mt-[-1.5rem] left-1/2 transform -translate-x-1/2 w-full max-w-4xl border rounded-t-[35px] p-4 sm:p-5">
          <div className="mb-4 mt-3">
            ...
            <div className="flex flex-wrap items-center mt-1 gap-2">
              <div className="flex items-center">
                ...
                {loading ? <Skeleton className="h-3 w-10 ml-2" /> : null}
              </div>
              {loading ? <Skeleton className="h-3 w-24" /> : null}
              {loading ? <Skeleton className="h-5 w-16 rounded-full" /> : null}
            </div>

            {/* Location */}
            <div className="flex items-center mt-2 text-[#959494] font-light text-sm">
              <MapPin size={14} className="mr-1 text-[#28B872]" />
              {loading ? <Skeleton className="h-3 w-24" /> : locationText ? <span>{locationText}</span> : null}
            </div>
            ...
            <div className="absolute top-7 right-5 rounded-lg p-1 sm:p-3 text-right z-10">
              {loading ? (
                ...
                <>
                  ...
                  {priceNowStr ? <div className="text-green-600 font-bold text-lg sm:text-xl">{priceNowStr}</div> : null}
                  {priceOldStr ? (
                    <div className="text-[#A6A6A6] text-xs sm:text-sm line-through mr-2 sm:mr-6">{priceOldStr}</div>
                  ) : null}
          ...
          <div className="mb-6 border border-[#E9F4F4] pl-8 sm:px-10 py-3 sm:py-5 rounded-[35px] flex justify-between sm:gap-0">
            <div className="min-w-[80px]">
              ...
              {loading ? <Skeleton className="h-4 w-12" /> : durationDays > 0 ? <div className="font-medium text-[#28B872]">{durationDays} Days</div> : null}
            </div>

            <div className="min-w-[80px]">
              <div className="text-xs text-[#959494]">Group Size</div>
              {loading ? <Skeleton className="h-4 w-10" /> : typeof groupSize === "number" ? (
                <div className="font-medium text-[#28B872]">{groupSize} {groupSize === 1 ? "person" : "people"}</div>
              ) : null}
            </div>
          ...
          <div className="shadow-xl bg-white flex flex-col rounded-[35px] p-3 sm:p-4">
              ...
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex gap-2 items-center">
                    <h3 className="font-bold text-sm sm:text-base truncate">Abebe Balcha</h3>
                    <BadgeCheck strokeWidth={3} className="text-green-700 w-4 h-4 sm:w-5 sm:h-5" />
                  </div>
                  <p className="text-gray-500 text-xs sm:text-sm truncate">18 years experience</p>
                </div>
              </div>

              <div className="flex flex-col items-end gap-1 min-w-[120px]">
                <span className="text-[#959494] font-semibold text-xs sm:text-sm truncate">Your Expert Guide</span>
            ...
            <div className="ml-12">
              <p className="text-[#959494] text-justify font-bold text-xs sm:text-sm px-2 sm:px-16 mb-4 max-w-[400]">
                Carlos is a naturalist guide born in the Amazon basin. His encyclopedic knowledge of rainforest ecology combined with his ability to spot even the most elusive wildlife, makes him one of Perus most respected jungle guides.
              </p>
            </div>
          ...
          ) : Images.length || title || locationText || priceNowStr ? (
            <TravelCard
              imageUrl={Images[0] || "/tipsimage.png"}
              placeName={(title.split(",")[0] || "").trim()}
              location={locationText}
              tripDuration={durationDays > 0 ? `${durationDays} day's trip` : ""}
              price={priceNowStr}
